...
exports.user_post = function (req, res, next) {
  ...
  .exec(function(err, newFriend) {
    ...
    const friendReqMadeIndex = newFriend.friend_received_req.indexOf(res.locals.currentUser._id);
    if (friendReqMadeIndex != -1) {
      var err = new Error("You have already sent a friend request to this person!");
      err.status = 404;
      return next(err);
    }
    // No real reason why this is done instead of checking friend_received_req instead. They seem equivalent.
    const friendReqIndex = newFriend.friend_given_req.indexOf(res.locals.currentUser._id);
    if (friendReqIndex != -1) {
      ...
      async.parallel(
      {
        user: function firstUser(callback) {
          User.findByIdAndUpdate(res.locals.currentUser._id, { friend_given_req: res.locals.currentUser.friend_given_req }).exec(callback);
        },
        otherUser: function otherUser(callback) {
          User.findByIdAndUpdate(newFriend._id, { friend_received_req: newFriend.friend_received_req }).exec(callback);
        },
      }, 
...
exports.friendAccept_post = function (req, res, next) {
  ...
  .exec(function(err, newFriend) {
    ...
    if (friendReqIndex == -1) {
      ...
      const currentUserFriendIndex = res.locals.currentUser.friend_received_req.indexOf(req.params.id);
      res.locals.currentUser.friend_received_req.splice(currentUserFriendIndex, 1);
      ...
      {
        user: function firstUser(callback) {
          User.findByIdAndUpdate(res.locals.currentUser._id, newCurrentUser).exec(callback);
        },
        otherUser: function otherUser(callback) {
...
exports.friendReject_post = function (req, res) {
  ...
  .exec(function(err, newFriend) {
    ...
    if (friendReqIndex == -1) {
      ...
      {
        user: function firstUser(callback) {
          User.findByIdAndUpdate(res.locals.currentUser._id, { friend_received_req: res.locals.currentUser.friend_received_req }).exec(callback);
        },
        otherUser: function otherUser(callback) {
          User.findByIdAndUpdate(newFriend._id, { friend_given_req: newFriend.friend_given_req }).exec(callback);
        },
      }, 
      function(err, results) {
        if (err) {
...
exports.profilePic = function (req, res, next) {
  ...
  User.findByIdAndUpdate(res.locals.currentUser._id, { profile_pic_url: picUrl })
